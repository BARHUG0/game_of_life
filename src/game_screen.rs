//game_screen.rs
use crate::maze_generator::LevelSize;
use raylib::prelude::*;

/// Current game screen state
#[derive(Debug, Clone, Copy, PartialEq)]
pub enum GameScreen {
    MainMenu,
    LevelSelect,
    Playing,
    Victory,
    GameOver,
}

pub struct ScreenRenderer {
    screen_width: i32,
    screen_height: i32,
    menu_background: Option<Texture2D>,
    gameover_background: Option<Texture2D>,
    selected_level: usize, // 0 = Small, 1 = Medium, 2 = Large
}

impl ScreenRenderer {
    pub fn new(
        screen_width: i32,
        screen_height: i32,
        menu_background: Option<Texture2D>,
        gameover_background: Option<Texture2D>,
    ) -> Self {
        ScreenRenderer {
            screen_width,
            screen_height,
            menu_background,
            gameover_background,
            selected_level: 0,
        }
    }

    pub fn move_selection_up(&mut self) {
        if self.selected_level > 0 {
            self.selected_level -= 1;
        }
    }

    pub fn move_selection_down(&mut self) {
        if self.selected_level < 2 {
            self.selected_level += 1;
        }
    }

    pub fn get_selected_level(&self) -> LevelSize {
        match self.selected_level {
            0 => LevelSize::Small,
            1 => LevelSize::Medium,
            _ => LevelSize::Large,
        }
    }

    pub fn render_main_menu(&self, d: &mut RaylibDrawHandle) {
        // Same as before...
        if let Some(bg) = &self.menu_background {
            d.draw_texture_pro(
                bg,
                Rectangle::new(0.0, 0.0, bg.width as f32, bg.height as f32),
                Rectangle::new(
                    0.0,
                    0.0,
                    self.screen_width as f32,
                    self.screen_height as f32,
                ),
                Vector2::zero(),
                0.0,
                Color::WHITE,
            );
        } else {
            d.clear_background(Color::new(20, 20, 30, 255));
        }

        d.draw_rectangle(
            0,
            0,
            self.screen_width,
            self.screen_height,
            Color::new(0, 0, 0, 150),
        );

        let title = "WOLFENSTEIN CLONE";
        let title_size = 80;
        let title_width = d.measure_text(title, title_size);
        let title_x = (self.screen_width - title_width) / 2;
        let title_y = self.screen_height / 4;

        d.draw_text(title, title_x + 3, title_y + 3, title_size, Color::BLACK);
        d.draw_text(title, title_x, title_y, title_size, Color::RED);

        let prompt = "Press ENTER to start";
        let prompt_size = 40;
        let prompt_width = d.measure_text(prompt, prompt_size);
        let prompt_x = (self.screen_width - prompt_width) / 2;
        let prompt_y = self.screen_height * 3 / 4;

        d.draw_text(
            prompt,
            prompt_x + 2,
            prompt_y + 2,
            prompt_size,
            Color::BLACK,
        );
        d.draw_text(prompt, prompt_x, prompt_y, prompt_size, Color::YELLOW);
    }

    pub fn render_level_select(&self, d: &mut RaylibDrawHandle) {
        if let Some(bg) = &self.menu_background {
            d.draw_texture_pro(
                bg,
                Rectangle::new(0.0, 0.0, bg.width as f32, bg.height as f32),
                Rectangle::new(
                    0.0,
                    0.0,
                    self.screen_width as f32,
                    self.screen_height as f32,
                ),
                Vector2::zero(),
                0.0,
                Color::WHITE,
            );
        } else {
            d.clear_background(Color::new(20, 20, 30, 255));
        }

        d.draw_rectangle(
            0,
            0,
            self.screen_width,
            self.screen_height,
            Color::new(0, 0, 0, 150),
        );

        // Title
        let title = "SELECT DIFFICULTY";
        let title_size = 70;
        let title_width = d.measure_text(title, title_size);
        let title_x = (self.screen_width - title_width) / 2;
        let title_y = self.screen_height / 4;

        d.draw_text(title, title_x + 3, title_y + 3, title_size, Color::BLACK);
        d.draw_text(title, title_x, title_y, title_size, Color::ORANGE);

        // Level options
        let levels = vec![
            ("SMALL", "21x21 maze - 20 enemies"),
            ("MEDIUM", "29x29 maze - 35 enemies"),
            ("LARGE", "35x35 maze - 50 enemies"),
        ];

        let mut y = self.screen_height / 2 - 60;
        let font_size = 45;
        let sub_size = 25;

        for (i, (name, desc)) in levels.iter().enumerate() {
            let is_selected = i == self.selected_level;
            let color = if is_selected {
                Color::YELLOW
            } else {
                Color::WHITE
            };
            let shadow_offset = if is_selected { 3 } else { 2 };

            // Draw selection indicator
            if is_selected {
                let indicator = "> ";
                let ind_width = d.measure_text(indicator, font_size);
                let ind_x = self.screen_width / 2 - 250;
                d.draw_text(indicator, ind_x + 2, y + 2, font_size, Color::BLACK);
                d.draw_text(indicator, ind_x, y, font_size, Color::YELLOW);
            }

            // Draw level name
            let name_width = d.measure_text(name, font_size);
            let name_x = (self.screen_width - name_width) / 2;
            d.draw_text(
                name,
                name_x + shadow_offset,
                y + shadow_offset,
                font_size,
                Color::BLACK,
            );
            d.draw_text(name, name_x, y, font_size, color);

            // Draw description
            let desc_width = d.measure_text(desc, sub_size);
            let desc_x = (self.screen_width - desc_width) / 2;
            d.draw_text(desc, desc_x + 1, y + 50 + 1, sub_size, Color::BLACK);
            d.draw_text(desc, desc_x, y + 50, sub_size, Color::GRAY);

            y += 120;
        }

        // Controls hint
        let hint = "UP/DOWN to select - ENTER to confirm";
        let hint_size = 30;
        let hint_width = d.measure_text(hint, hint_size);
        let hint_x = (self.screen_width - hint_width) / 2;
        let hint_y = self.screen_height * 4 / 5;

        d.draw_text(hint, hint_x + 2, hint_y + 2, hint_size, Color::BLACK);
        d.draw_text(
            hint,
            hint_x,
            hint_y,
            hint_size,
            Color::new(150, 150, 150, 255),
        );
    }

    /// Render the game over screen directly to the screen
    pub fn render_game_over(&self, d: &mut RaylibDrawHandle, score: i32, kills: i32) {
        // Draw background if available
        if let Some(bg) = &self.gameover_background {
            d.draw_texture_pro(
                bg,
                Rectangle::new(0.0, 0.0, bg.width as f32, bg.height as f32),
                Rectangle::new(
                    0.0,
                    0.0,
                    self.screen_width as f32,
                    self.screen_height as f32,
                ),
                Vector2::zero(),
                0.0,
                Color::WHITE,
            );
        } else {
            // Fallback: dark red background
            d.clear_background(Color::new(40, 10, 10, 255));
        }

        // Semi-transparent overlay
        d.draw_rectangle(
            0,
            0,
            self.screen_width,
            self.screen_height,
            Color::new(0, 0, 0, 180),
        );

        // Game Over title
        let title = "GAME OVER";
        let title_size = 90;
        let title_width = d.measure_text(title, title_size);
        let title_x = (self.screen_width - title_width) / 2;
        let title_y = self.screen_height / 3;

        d.draw_text(title, title_x + 3, title_y + 3, title_size, Color::BLACK);
        d.draw_text(
            title,
            title_x,
            title_y,
            title_size,
            Color::new(200, 50, 50, 255),
        );

        // Stats
        let font_size = 40;
        let score_text = format!("Final Score: {}", score);
        let kills_text = format!("Enemies Killed: {}", kills);

        let mut y = self.screen_height / 2 + 20;

        // Score
        let score_width = d.measure_text(&score_text, font_size);
        let score_x = (self.screen_width - score_width) / 2;
        d.draw_text(&score_text, score_x + 2, y + 2, font_size, Color::BLACK);
        d.draw_text(&score_text, score_x, y, font_size, Color::ORANGE);

        y += 60;

        // Kills
        let kills_width = d.measure_text(&kills_text, font_size);
        let kills_x = (self.screen_width - kills_width) / 2;
        d.draw_text(&kills_text, kills_x + 2, y + 2, font_size, Color::BLACK);
        d.draw_text(
            &kills_text,
            kills_x,
            y,
            font_size,
            Color::new(200, 200, 200, 255),
        );

        // Restart prompt
        let prompt = "Press ENTER to restart";
        let prompt_size = 35;
        let prompt_width = d.measure_text(prompt, prompt_size);
        let prompt_x = (self.screen_width - prompt_width) / 2;
        let prompt_y = self.screen_height * 3 / 4;

        d.draw_text(
            prompt,
            prompt_x + 2,
            prompt_y + 2,
            prompt_size,
            Color::BLACK,
        );
        d.draw_text(prompt, prompt_x, prompt_y, prompt_size, Color::YELLOW);
    }

    pub fn render_victory(&self, d: &mut RaylibDrawHandle, score: i32, kills: i32, time: &str) {
        if let Some(bg) = &self.menu_background {
            d.draw_texture_pro(
                bg,
                Rectangle::new(0.0, 0.0, bg.width as f32, bg.height as f32),
                Rectangle::new(
                    0.0,
                    0.0,
                    self.screen_width as f32,
                    self.screen_height as f32,
                ),
                Vector2::zero(),
                0.0,
                Color::WHITE,
            );
        } else {
            d.clear_background(Color::new(10, 30, 10, 255));
        }

        d.draw_rectangle(
            0,
            0,
            self.screen_width,
            self.screen_height,
            Color::new(0, 0, 0, 180),
        );

        // Victory title
        let title = "MISSION COMPLETE!";
        let title_size = 90;
        let title_width = d.measure_text(title, title_size);
        let title_x = (self.screen_width - title_width) / 2;
        let title_y = self.screen_height / 4;

        d.draw_text(title, title_x + 3, title_y + 3, title_size, Color::BLACK);
        d.draw_text(
            title,
            title_x,
            title_y,
            title_size,
            Color::new(50, 200, 50, 255),
        );

        // Stats
        let font_size = 40;
        let stats = vec![
            format!("Final Score: {}", score),
            format!("Enemies Killed: {}", kills),
            format!("Time: {}", time),
        ];

        let mut y = self.screen_height / 2 - 20;

        for stat in stats {
            let stat_width = d.measure_text(&stat, font_size);
            let stat_x = (self.screen_width - stat_width) / 2;
            d.draw_text(&stat, stat_x + 2, y + 2, font_size, Color::BLACK);
            d.draw_text(&stat, stat_x, y, font_size, Color::GOLD);
            y += 60;
        }

        // Restart prompt
        let prompt = "Press ENTER to continue";
        let prompt_size = 35;
        let prompt_width = d.measure_text(prompt, prompt_size);
        let prompt_x = (self.screen_width - prompt_width) / 2;
        let prompt_y = self.screen_height * 3 / 4;

        d.draw_text(
            prompt,
            prompt_x + 2,
            prompt_y + 2,
            prompt_size,
            Color::BLACK,
        );
        d.draw_text(prompt, prompt_x, prompt_y, prompt_size, Color::YELLOW);
    }
}
